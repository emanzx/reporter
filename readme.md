# Grafana reporter <img style="float: right;" src="https://travis-ci.org/emanzx/reporter.svg?branch=master">
...
Get the source files and dependencies:
    ...
    go get github.com/emanzx/reporter/...

Build and install `grafana-reporter` binary to `$GOPATH/bin`:

    go install -v github.com/emanzx/reporter/cmd/grafana-reporter
...
Also see [this issue](https://github.com/emanzx/reporter/issues/50) for an example. 
...
A Docker image [is available](https://hub.docker.com/r/emanzx/grafana-reporter/). To see available flags:

    docker run emanzx/grafana-reporter --help

To run with default flags, use `--net` to enable Docker to connect to Grafana at `localhost:3000`:

    docker run -p 8686:8686 --net="host" emanzx/grafana-reporter
...
If you also have `Make` and `Docker-compose` installed, you can run a simple local orchestration of Grafana and Grafana-reporter:
     ...
     go get github.com/emanzx/reporter/ ...
     cd $GOPATH/src/github.com/emanzx/reporter
     make compose-up
...
The unit tests can be run using the go tool:
    ...
    go test -v github.com/emanzx/reporter/...

or, the [GoConvey](http://goconvey.co/) webGUI:

    ./bin/goconvey -workDir `pwd`/src/github.com/emanzx -excludedDirs `pwd`/src/github.com/emanzx/reporter/tmp/
